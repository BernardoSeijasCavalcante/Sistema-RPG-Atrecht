...
import model.resources.effects.Strength;
import model.resources.effects.Vulnerability;

public class StrengthEffectButton extends EffectsButtons{

	public StrengthEffectButton() {
		super(new Image("resources\\Effects\\strength.png"));
	...
	public void clickEvent(MouseEvent e) {
		if(getTarget().contains(e.getX(), e.getY())) {
			if(!isActivity()) {
				...
				int strengthA = (int) (championReceiver.getStrength() * (0.1 + 0.2 * Math.random()));
				
				int played = (int) (3 * Math.random());
				
				setEffect(new Strength(0,0,strengthA,0,0,0,0,played));
